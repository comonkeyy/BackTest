...
exports.getAllUsers = (req, res, next) => {
  ...
  exports.getUserDetail = (req, res, next) => {
    ...
    userModel.getById(userId, (err, user) => {
      if (err) return next(err);
      if (!user) return res.status(404).json({ success: false, message: "계정을 찾을 수 없습니다." });
  ...
  exports.createUser = (req, res, next) => {
    ...
    userModel.create({ user_id, password, name, email, phone, role }, (err, result) => {
      if (err) return next(err);
  ...
  exports.updateUser = (req, res, next) => {
    ...
    userModel.update(userId, updateData, (err, result) => {
      if (err) return next(err);
      if (result.affectedRows === 0) return res.status(404).json({ success: false, message: "계정을 찾을 수 없습니다." });
